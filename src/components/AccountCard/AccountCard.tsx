...
const getAccountDetails = (account: Account) => {
    ...
    if (account.type === AccountType.Gas)
        return `Volume: ${account.volume}`;
}
...
export default function AccountCard({ account, balance, onMakePayment }: Props) {
    ...
    return (
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition hover:shadow-md">
            <div className="flex items-start gap-4">
                {/* Icon */}
                <div className={`h-12 w-12 rounded-full border flex items-center justify-center text-base bg-blue-50 border-blue-200`}
                    aria-hidden>
                    {getIcon(type)}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-4 flex items-center justify-between">
                        ...
                        <div className={`tabular-nums font-semibold ${getAmountColor(balance)}`}>
